...
//		p1.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
		...
		p2.setBorder(new TitledBorder(new LineBorder(new Color(1, 92, 173)), "노트북")); // p2에 테두리와 타이틀 생성
		String header[] = { "제품코드", "크기", "무게", "RAM용량", "배터리", "가격", "운영체제", "제조사" };
...
//			SearchPW(brand);
			...
			String sql = "SELECT * FROM NOTEBOOK WHERE BRAND ='" + brand + "'";
			ResultSet rs = db.DB_Lib.getResultSet(sql);
	...
	private void DrawTable(ResultSet rs) { // 테이블 그리는 메소드
		try {
			while (rs.next()) { // DB이용해 테이블 생성
				...
				String lib_inch = rs.getString(2);
				String lib_weight = rs.getString(3);
				String lib_ram = rs.getString(4);
				String lib_battery = rs.getString(5);
				int lib_price = rs.getInt(6);
				String lib_operate = rs.getString(7);
				String lib_brand = rs.getString(8);

				Object data[] = { lib_code, lib_inch, lib_weight, lib_ram, lib_battery, lib_price, lib_operate,
						lib_brand };
	...
	public Boolean AddCart() {
		checkAddCart = false;
		String sql = "SELECT CODE, PRICE, NAME FROM NOTEBOOK";
		ResultSet rs = db.DB_Lib.getResultSet(sql);
		try {
