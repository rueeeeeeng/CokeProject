...
public class SelectMonitor extends JFrame implements ActionListener, KeyListener {
	...
	public SelectMonitor(String title, int width, int height) {
		setTitle(title);
...
//		p1.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
		...
		p2.setBorder(new TitledBorder(new LineBorder(Color.GRAY), "모니터")); // p2에 테두리와 타이틀 생성
		String header[] = { "제품코드", "제조사", "모델명", "해상도", "무게", "가격" };
...
//			SearchPW(brand);
			...
			String sql = "SELECT * FROM MONITOR WHERE BRAND ='" + brand + "'";
			ResultSet rs = db.DB_Lib.getResultSet(sql);
	...
	private void DrawTable(ResultSet rs) { // 테이블 그리는 메소드
		try {
			while (rs.next()) { // DB이용해 테이블 생성
				...
				String lib_brand = rs.getString(2);
				String lib_name = rs.getString(3);
				String lib_resolution = rs.getString(4);
				String lib_weight = rs.getString(5);//
				String lib_price = rs.getInt(6)+"원";//

				Object data[] = { lib_code, lib_brand, lib_name, lib_resolution, lib_weight, lib_price };
	...
	public Boolean AddCart() {
		checkAddCart = false;
		String sql = "SELECT CODE, PRICE, NAME FROM MONITOR";
		ResultSet rs = db.DB_Lib.getResultSet(sql);
		try {
