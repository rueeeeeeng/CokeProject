...
public class SelectMouse extends JFrame implements ActionListener, KeyListener {
	...
	public SelectMouse(String title, int width, int height) {
		setTitle(title);
...
//		p1.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
		...
		p2.setBorder(new TitledBorder(new LineBorder(Color.GRAY), "마우스")); // p2에 테두리와 타이틀 생성
		String header[] = { "제품코드", "제조사", "제품이름", "연결방식", "가격" };
...
//			SearchPW(brand);
			...
			String sql = "SELECT * FROM MOUSE WHERE BRAND ='" + brand + "'";
			ResultSet rs = db.DB_Lib.getResultSet(sql);
	...
	private void DrawTable(ResultSet rs) { // 테이블 그리는 메소드
		try {
			while (rs.next()) { // DB이용해 테이블 생성
				...
				Object data[] = { code, brand, name, connectway, price };
	...
	public Boolean AddCart() {
		checkAddCart = false;
		String sql = "SELECT CODE, PRICE, NAME FROM MOUSE";
		ResultSet rs = db.DB_Lib.getResultSet(sql);
		try {
