...
public class MouseAdd extends JFrame implements ActionListener{
   ...
   public MouseAdd(String title, int width, int height) {
      setTitle(title);
   ...
   private void MainAdd() {
      ...
      p2.setBorder(new TitledBorder(new LineBorder(new Color(1, 92, 173), 2), " 상품 : 마우스 "));
   }
      ...
      if(obj==b1) {
         ...
         if(tf1.getText().equals("") ||  tf2.getText().equals("") || tf3.getText().equals("") || tf4.getText().equals("") ||
            ...
            if(check) {
               ...
               String connectway = tf4.getText();
               String price = tf5.getText();
               int bestSeller  = 0;
               
                  String UpdateSql = "INSERT INTO MOUSE VALUES" + "('" + code + "', '" + brand + "', '" + pname + "', '" +
                  connectway + "', '" +  price + "', '" + bestSeller +  "')";
   ...
   private boolean overlap(String code) {
      boolean check = false;
      String sql = "SELECT CODE FROM MOUSE WHERE CODE = '" + code + "'";
